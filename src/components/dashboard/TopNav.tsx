...
export default function TopNav({ user }: { user: User }) {
    ...
    return (
        <header className="h-[46px] w-full bg-[#131722] border-b border-[#2A2E39] flex items-center justify-between px-4 z-30 shrink-0">
            {/* Left section: Logo + Search */}
            <div className="flex items-center gap-6 h-full">
                {/* Logo */}
                <Link href="/dashboard" className="flex items-center gap-2 pr-4 border-r border-[#2A2E39] h-full">
                    <Activity className="w-5 h-5 text-indigo-500" />
                    <span className="font-bold text-white text-[15px]">StockPulse</span>
                </Link>
                ...
                <div ref={wrapperRef} className="relative w-[280px]">
                    ...
                    <Input
                        ...
                        className="pl-8 bg-[#1E222D] border-transparent hover:border-[#2A2E39] text-[#D1D4DC] placeholder:text-[#A3A6AF] focus:border-indigo-500/50 h-8 text-[13px] rounded"
                        aria-label="Search markets"
                        id="stock-search"
                    />
                    {open && (
                        <div className="absolute top-[calc(100%+4px)] left-0 w-[400px] bg-[#1E222D] border border-[#2A2E39] rounded-lg shadow-2xl z-50">
                            {results.map((r) => (
                                <button
                                    key={r.symbol}
                                    onClick={() => navigate(r.symbol)}
                                    className="w-full flex items-center justify-between px-4 py-2 hover:bg-[#2A2E39] transition-colors text-left"
                                >
                                    <div className="flex items-baseline gap-2">
                                        <span className="text-[13px] font-bold text-[#D1D4DC]">{r.symbol}</span>
                                        <span className="text-[11px] text-[#A3A6AF]">{r.name}</span>
                                    </div>
                                    <span className="text-[10px] text-[#A3A6AF] bg-[#2A2E39] px-1.5 py-0.5 rounded">{r.type}</span>
                                </button>
                            ))}
                        </div>
                    )}
